<template>
	...
	<div class="d-flex flex-column">
		...
		<div v-for="(d, index) in personnage.atouts" :key="d" class="p-grid">
			...
			<div class="p-col">
				<Dropdown v-model="this.personnage.atouts[index]"
									:options="this.atouts"
									optionLabel="nom"
									optionValue=""/>
			</div>
			...
			<Button icon="pi pi-eye" class="p-button-info" @click="openModal(this.personnage.atouts[index])"/>


			<Dialog :header="this.selected.nom" v-model:visible="displayModal" :modal="true">
				<div class="p-field">
					<label for="atout_nom">Nom:</label>
					<InputText id="atout_nom" :value="this.selected.nom" type="text" disabled/>
				</div>

				<div class="p-formgrid p-grid">
					<div class="p-field p-col">
						<label for="atout_type">Type:</label>
						<InputText id="atout_type" :value="this.selected.type" type="text" disabled/>
					</div>
					<div class="p-field p-col">
						<label for="atout_valeur">Valeur:</label>
						<InputText id="atout_valeur" :value="this.selected.valeur" type="text" disabled/>
					</div>
				</div>

				<div class="p-field">
					<label for="atout_description">Description:</label>
					<Textarea id="atout_description" :value="this.selected.description" :autoResize="true" rows="4" cols="30" disabled/>
				</div>

				<div class="p-field">
					<label for="atout_description">Effet:</label>
					<Textarea id="atout_effet" :value="this.selected.effet" :autoResize="true" rows="4" cols="30" disabled/>
				</div>
